...
It is basically used to transform a array, map() function in JavaScript is a method used on arrays to transform each element of the array and return a new array with the transformed values.

const output = arr.map(_function_) // this _function_ tells map that what transformation I want on each element of array

```js
const arr = [5, 1, 3, 2, 6];
// Double the array element: [10, 2, 6, 4, 12]
function double(x) {
  return x * 2;
}
...
// like: {29 : 2, 75 : 1, 50 : 1}
// We should use reduce, why? we want to reduce some information from the array. Basically we want to get a single object as output
